...
let
  ...
  # build.nix is intentionally self-contained. The wheel contains the exact
  # public headers and libtvm_ffi runtime for the ABI recorded in TargetSpec.
  ...
  cudaPackages =
    if builtins.hasAttr "cudaPackages_12_8" pkgs then
      pkgs.cudaPackages_12_8
    else
      throw "the locked nixpkgs does not provide cudaPackages_12_8";

  # Local build API v1. This wrapper is frozen into each Kernel Release and
  # makes the only-output invariant explicit instead of relying on a kdlib.
  mkTvmFfiKernel =
